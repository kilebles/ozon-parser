"""
Main entry point using Selenium with Chrome/Edge.

Uses Selenium WebDriver with comprehensive anti-detection features.
Supports Chrome and Edge browsers with auto-driver download.
"""
import asyncio
import random
import sys
...
async def run_tracker() -> None:
    ...
    logger.info(f"Starting Selenium tracker for {len(spreadsheet_ids)} spreadsheets")

    try:
        async with OzonParserSelenium() as parser:
            # Process spreadsheets sequentially (Selenium uses single driver)
